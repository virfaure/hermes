...
class Magmi_DefaultAttributeItemProcessor extends Magmi_ItemProcessor
{
	...
	public function handleVarcharAttribute($pid,&$item,$storeid,$attrcode,$attrdesc,$ivalue)
	{
		...
		//if it's a multiselect value
                if($attrcode != "accesscontrol_show_group"){
                    ...
                    // accesscontrol_show_group is a multiselect with no options !!
                    // options are loaded by the module accesscontrol/config_source_customergroups_product
                    if(HermesHelper::getMagentoIsActive(HermesHelper::AC_MODULE_FILE)){
                        require_once HermesHelper::getMagentoRootPath().'/app/Mage.php';
                        Mage::app();

                        $sep=Magmi_Config::getInstance()->get("GLOBAL","multiselect_sep",",");
                        $arrCustomerGroup=explode($sep,$ivalue);

                        foreach($arrCustomerGroup as $customerGroup){
                            $customerGroup = trim($customerGroup);

                            //Get Values of Customer Groups
                            $customerGroupId = constant("ZetaPrints_AccessControl_Helper_Data::$customerGroup");
                            if($customerGroupId){
                                //catalog_product_entity_varchar
                                $arrCustomerGroupString[] = $customerGroupId;
                            }else{
                                //check if it is a customer group from magento
                                $group = Mage::getModel('customer/group')->getCollection()->addFieldToFilter('customer_group_code', $customerGroup)->getFirstItem();
                                if($group->getId()){
                                    $arrCustomerGroupString[] = $group->getId();
                                }else{
                                    $this->log("ERROR at sku {$item['sku']} - Customer Group Specified [$customerGroup] does not exists","warning");
                                }
                            }
                        }

                        if(!empty($arrCustomerGroupString))
                            $ovalue=implode(",",$arrCustomerGroupString);
                     }
                }
		return $ovalue;
	}
