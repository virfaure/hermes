...
class ImageAttributeItemProcessor extends Magmi_ItemProcessor
{
    ...
    public function initialize($params)
    {
        ...
        {
            $hostname = HermesHelper::getProject();
            $this->imgsourcedir = realpath($this->magdir . '/hermes/upload/' . $hostname);
            if($this->imgsourcedir == false) {
                $this->imgsourcedir = $this->getParam("IMG:sourcedir");
            }
        }
        $this->log('IMG SOURCE FOLDER:' . $this->imgsourcedir);
     ...
     * @param string $imgname : image file name (relative to /products/media in magento dir)
     */
    public function getImageValue($storeid,$imageValueId)
    {
        $tgv=$this->tablename('catalog_product_entity_media_gallery_value');
        $sql=" SELECT value_id FROM `$tgv` WHERE store_id=? AND value_id=?";
        return $this->selectone($sql,array($storeid,$imageValueId),'value_id');
    }
    ...
    public function addImageToGallery($pid,$storeid,$attrdesc,$imgname,$targetsids,$imglabel=null,$excluded=false,$refid=null)
    {
        ...
        if($vid!=null)
        {
            ...
            if(!$this->getImageValue($storeid, $vid)){
                ...
                $tv=$this->tablename('catalog_product_entity_media_gallery_value');
                $sql="UPDATE $tv SET disabled=?
                    WHERE value_id=? and store_id =?";
                $this->update($sql,array($excluded?1:0,$vid,$storeid));
            }
        }
    }
    ...
    private function resizeImage($target) {
        ...
        $maxW = $this->getParam("IMG:resizingw");
        $maxH = $this->getParam("IMG:resizingh");
        if($width > $maxW && $maxW > 0) {
            $image = new SimpleImage();
            $image->load($target);
            $image->resizeToWidth($maxW);
            $image->save($target);
        } else if ($height > $maxH && $maxH > 0) {
            $image = new SimpleImage();
            $image->load($target);
    ...
    public function copyImageFile($imgfile,&$item,$extra)
    {
        ...
        if(!file_exists("$te") || $this->getParam("IMG:writemode")=="override")
        {
            ...
            if(!$exists)
            {
                $this->log("<b>For product with sku: {$item['sku']}</b>, image '$fname' not found, this product might be shown <b>WITHOUT</b> image.","warning");
                $this->fillErrorAttributes($item);
            ...
            if($exists)
            {
                ...
                if($this->getParam("IMG:resizing") == 'yes') {
                    $this->resizeImage("$l2d/$bimgfile");
                }
                $this->destroyUrlContext($curlh);
    ...
    public function processItemBeforeId(&$item,$params=null)
    {
        ...
        //Media Gallery
        if(!empty($item["image_label"]) && !empty($item["media_gallery"])){
            $images=explode(",",$item["media_gallery"]);

            foreach($images as &$imagefile)
            {
                $imagefile .="::".$item["image_label"];
            }

            $item["media_gallery"] = implode(",", $images);
        }
        ...
        if(isset($item["image"])){
            ...
            if($countImage > 0){
                ...
                foreach($images as $imagefile)
                {
                    ...
                    //Check if imagefile has extension
                    $ext = "";
                    $ext = substr(strrchr($imagefile, '.'), 1);
                    if(empty($ext)){
                        // No extension, try to guess with directory files
                        $dir = $this->imgsourcedir;
                        if(!file_exists($dir)) {
                            ...
                            while (false !== ($filename = readdir($dh))) {
                                ...
                                if (preg_match("/$imagefile\.(.*)/", $filename, $matches)) {
                                    $ext = $matches[1];
                                    break;
                                }

                                                /*$pos = strpos($filename, $imagefile);

                                                if ($pos === false) {
                                                    //$this->log("ext : $imagefile ".$filename, "virge");
                                                } else {
                                                    $this->log("find : $imagefile ".$filename, "virge");
                                                    break;
                                                }*/
                            }
                            closedir($dh);
                    ...
                    if($cpt == 1){
                        ...
                        if(empty($item["media_gallery"])) $item["media_gallery"] = null;
                    }else{
                        if(!empty($item["image_label"])) $imagefile .="::".$item["image_label"];
        ...
        // If media_gallery contains +/-, put media_gallery_reset a 0
    ...
    public function processColumnList(&$cols,$params=null)
    {
        ...
        $colsTemp = array_flip($cols);

        if(isset($colsTemp["image"])){
            $cols[] = "small_image";
            $cols[] = "thumbnail";
        }
