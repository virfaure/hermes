...
class Magmi_ProductImportEngine extends Magmi_Engine
{
	...
	private $_skustats=array();
        ...
        // Stock history
        private static $_isStockHistory;
        private $_tableStockHistory = 'cataloginventory_stock_history';

        // Aitoc Stock : Module for Stock by Website(Affinity)
        private static $_isAitocStock = false;

    public function isStockHistory()
	{

            self::$_isStockHistory = false;
	}
    
    public function isAitocStock()
	{
        $this->_isAitocStock =  HermesHelper::getMagentoIsActive(HermesHelper::AITOCSTOCK_MODULE_FILE);
	}
	...
	public function __construct()
	{
        ...
        // Check for aitoc multi-stock module
        $this->isAitocStock();
	}
	...
	{
		$tname=$this->tablename('catalog_product_entity');
		$type=$this->selectone("SELECT type_id FROM $tname WHERE sku=?", $sku, "type_id");
		if($type){
			return $type;
		}else return false;
	}
	...
	function  createOptionValue($optid,$store_id,$optval)
	{
            ...
            $optvalueOk = ucfirst(trim($optval));
            $optval_id=$this->insert("INSERT INTO $t (option_id,store_id,value) VALUES (?,?,?)",array($optid,$store_id,$optvalueOk));
	...
	function getOptionIds($attid,$storeid,$values)
	{
		...
		$new=array_merge(array_udiff($avalues,$exvals,'strcasecmp'));
		if($storeid==0)
		{
			foreach($new as $nval)
			{
				$row=array("opvs"=>$this->createOption($attid),"value"=>$nval);
				$this->createOptionValue($row["opvs"],$storeid,ucfirst($nval));
				$existing[]=$row;
			}
	...
	public function updateStock($pid,$item,$isnew)
    {
        ...
        // Prepare aitoc tables
        if ($this->_isAitocStock){
    		$aitoc_csit=$this->tablename("aitoc_cataloginventory_stock_item");
	    	$aitoc_css=$this->tablename("aitoc_cataloginventory_stock_status");
        }
		...
		if(count($common)>0)
		{
            ...
            $qtys = array();
            if(isset($item['qty'])) $qtys = explode(',', $item['qty']);
            ...
            while($i < count($qtys)){
                ...
                if (isset($stockvals['qty'])) $stockvals["qty"] = $qtys[$i];

    			//test for relative qty
	    		if(isset($qtys[$i]) && ($qtys[$i][0]=="+" || $qtys[$i][0]=="-"))
		    	{
                    $relqty=getRelative($qtys[$i]);
    			}
	    		//if relative qty
		    	if($relqty!=NULL)
			    {
                ...
                // insert or update also aitoc table 
                if ($this->_isAitocStock){
                    if ($item['type'] == 'confgurable') $stockvals['qty_increments'] = $stockvals['qty_increments'] ? $stockvals['qty_increments'] : 0;

                    //aitoc necesita un valor mas en stockvals
                    $stockvals['use_default_website_stock'] = 0;

                    $cols = implode(', ', array_keys($stockvals));
                    $interrogants = str_repeat(', ? ', count($stockvals));
                    $str_update = '';
                    foreach($stockvals as $key => $val){
                        $str_update .= "$key = ?, ";
                    }
                    $str_update = trim($str_update, ', ');

                    $aitoc_sql = "
                        INSERT INTO `$aitoc_csit` (website_id, product_id, stock_id, $cols) 
                        VALUES (?, ?, ? $interrogants) 
                        ON DUPLICATE KEY UPDATE $str_update
                        ";
                    $data = array_merge(array($wsids[$i], $pid, $stock_id), array_values($stockvals), array_values($stockvals));
                    $this->insert($aitoc_sql, $data);

                    //volvemos a dejar stockvals com estaba
                    unset($stockvals['use_default_website_stock']);
                }
                
                $i++;
            }

                        // If Module Stock History is installed, and we updated qty attribute
                        if(self::$_isStockHistory === null){
                            $this->isStockHistory();
                        }

                        if(self::$_isStockHistory === true){
                            $stockvalsHistory = array();

                            $stockvalsHistory['qty'] = $stockvals['qty'];
                            $stockvalsHistory['is_in_stock'] = $stockvals['is_in_stock'];

                            if($relqty!=NULL)
                            {
                                $stockParam = $this->selectAll("SELECT qty, is_in_stock FROM `$csit` WHERE product_id=? AND stock_id=? LIMIT 1", array($pid,$stock_id));

                                if(count($stockParam) > 0){
                                    $stockvalsHistory['qty'] = $stockParam[0]['qty'];
                                    $stockvalsHistory['is_in_stock'] = $stockParam[0]['is_in_stock'];
                                }
                            }
                            // Update Table of Stock History
                            $sql='DESCRIBE '.$this->tablename('cataloginventory_stock_history');
                            $rows = $this->selectAll($sql);
                            foreach($rows as $row){
                                $cishcols[]=$row['Field'];
                            }
                            ...
                            $data[] = "magmi";
                            $data[] = null;
                            $data[] = $stockvalsHistory['qty'];
                            $data[] = $stockvalsHistory['is_in_stock'];
                            $data[] = "Magmi Import/Update";
                            $data[] = date("Y-m-d H:i:s");

                            if(!empty($stockvalsHistory['qty'])){
                                $sqlHistory="INSERT INTO `cataloginventory_stock_history` (".implode(",",$cishcols).") VALUES (".$this->arr2values($cishcols).")";
                                $this->insert($sqlHistory,$data);
                            }
                        }
        ...
        while($i < count($qtys)){
            $sql = "
                ...
                ON DUPLICATE KEY UPDATE stock_status = ?, qty = ?
                ";
            $data[]=$pid;
            $data[]=$wsids[$i];
    		$data[]=$stock_id;
            ...
            // Insert also in aitoc table
            if ($this->_isAitocStock){
                $aitoc_sql = "
                    INSERT INTO `$aitoc_css` VALUES(?, ?, ?, ?, ?)
                    ON DUPLICATE KEY UPDATE stock_status = ?, qty = ?
                ";
	    	    $this->insert($aitoc_sql,$data);
            }
     ...
     *
     * -- Copied directly from magmi 1.7.18 to allow importing item positions in categories --
     * 
     * @param int $pid : product id
	 * @param array $item : attribute values for product indexed by attribute_code
	 */
	...
	/*
	public function handleImageColumn(&$item)
	{
		if(isset($item["image"])){
			...
			{
				//Fist Image is default image
				if($cpt == 1){
					$item["image"] = $imagefile;
					$item["small_image"] = $imagefile;
					$item["thumbnail"] = $imagefile;
				}else{
					$item["media_gallery"] .= $imagefile;
					if($cpt < $countImage) $item["media_gallery"] .= ",";
				}
	...
	*/
	...
	public function initImport($params)
	{
		...
		try
		{
			...
			$this->callPlugins("datasources,itemprocessors,general","startImport");
			$this->resetSkuStats();
		}
		catch(Exception $e)
		{
