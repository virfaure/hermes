...
$('runmagmi').observe('submit',function(ev){
	...
	if(ls.changed!==false)
	{
        ...
        $$('input[type="checkbox"].pl_general').invoke('observe','click',function(field) {
            if(this.checked){
                
                if(this.name == 'Magmi_ReindexingPlugin'){
                    $$('input[name="Magmi_ProductSavePlugin"]').invoke('setValue', '');
                }else if(this.name == 'Magmi_ProductSavePlugin'){
                    $$('input[name="Magmi_ReindexingPlugin"]').invoke('setValue', '');
                }
            }
        });	
    });
