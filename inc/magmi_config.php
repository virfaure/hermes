...
require_once dirname(__FILE__) . '/HermesHelper.php';
...
class DirbasedConfig extends Properties
{
        ...
        public function setDomainName()
	{
		...
		$domain = isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : $_SERVER['PWD'];

                $this->_domainName = "_".$domain;
	}
...
class ProfileBasedConfig extends DirbasedConfig
{
	...
	public function getProfileDir()
	{
		...
		// Check if Exists Perfil in domain directory
                $confdir=dirname(dirname(self::$_script)).DS."conf".DS.$this->_domainName.$subdir;
...
class Magmi_Config extends DirbasedConfig
{
	...
	public function getConfDir()
	{
        ...
        public function getDomainNameDir()
	{
            if(is_dir($this->getConfDir().DS.$this->_domainName)){
                return $this->getConfDir().DS.$this->_domainName;
            }else{
                return $this->getConfDir();
            }
	}
    ...
    {
		return HermesHelper::getMagentoRootPath() . '/';
	}
	...
	public function getProfileList()
	{
		...
		{
                    //If exists directory for site
                    if(is_dir($this->getConfDir().DS.$this->_domainName)){
                        $confdir = $this->getConfDir().DS.$this->_domainName.DS;
                    }else{
                        $confdir = $this->getConfDir().DS;
                    }

                    if(is_dir($confdir.DS.$candidate) && $candidate[0]!="." && substr($candidate,0,2)!="__" && substr($candidate,0,1)!="_")
                    {
                            $proflist[]=$candidate;
                    }
                }
