			...
			foreach($this->_props as $sec=>$data)
			{
				foreach($data as $k=>$v)
				{
					...
					$this->_props[$sec][$k]=$newv;
                        ...
                        if (preg_match("/magmi\.ini/i", $this->inifile)) {
                            if(count($this->_props["DATABASE"]) > 0){
                                $config = HermesHelper::getMagentoDBConfig();
                                if($config){
                                    $this->_props["DATABASE"]["host"] = (string)$config->host;
                                    $this->_props["DATABASE"]["user"] = (string)$config->username;
                                    $this->_props["DATABASE"]["password"] = (string)$config->password;
                                    $this->_props["DATABASE"]["dbname"] = (string)$config->dbname;
                                }
                            }
                        }
