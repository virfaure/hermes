...
require_once("magmi_categoryimportengine.php");

class Magmi_CategoryImport_DataPump
{
	...
	{
		$this->_engine=new Magmi_CategoryImportEngine();
	...
	public function endImportSession()
	{
		...
		$this->_engine->log("Categories imported OK:".$skustats["ok"]."/".$skustats["nsku"],"success");
		if($skustats["ko"]>0)
		{
			$this->_engine->log("Categories imported KO:".$skustats["ko"]."/".$skustats["nsku"],"warning");
		}
